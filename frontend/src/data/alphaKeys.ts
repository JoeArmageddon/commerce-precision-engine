// 50 Hardcoded Alpha Access Keys
// Each key can be used to create one account during alpha testing
...
// Interface for tracking used keys with metadata
...
/**
 * Get list of already used alpha keys
 */
...
/**
 * Check if an alpha key is valid
 */
export function isValidAlphaKey(key: string): boolean {
  const normalizedKey = key.trim().toUpperCase();
  return ALPHA_KEYS.includes(normalizedKey as any);
}

/**
 * Check if an alpha key has already been used
 */
export function isAlphaKeyUsed(key: string): boolean {
  const normalizedKey = key.trim().toUpperCase();
  const usedKeys = getUsedAlphaKeys();
  return usedKeys.some(u => u.key === normalizedKey);
}

/**
 * Mark an alpha key as used
 */
export function markAlphaKeyAsUsed(key: string, userId: string): void {
  const normalizedKey = key.trim().toUpperCase();
  const usedKeys = getUsedAlphaKeys();
  
  if (!usedKeys.some(u => u.key === normalizedKey)) {
    usedKeys.push({
      key: normalizedKey,
...
/**
 * Get remaining available alpha keys count
 */
...
/**
 * Get list of available (unused) alpha keys
 */
export function getAvailableAlphaKeys(): string[] {
  const usedKeys = getUsedAlphaKeys();
  const usedKeySet = new Set(usedKeys.map(u => u.key));
  return ALPHA_KEYS.filter(key => !usedKeySet.has(key));
}

/**
 * Reset all used keys (for testing only)
 */
...
export function validateAndUseAlphaKey(key: string, userId: string): { 
  ...
  if (!isValidAlphaKey(normalizedKey)) {
    return { 
      valid: false, 
  ...
  if (isAlphaKeyUsed(normalizedKey)) {
    return { 
      valid: false, 
  ...
  // Mark as used
  markAlphaKeyAsUsed(normalizedKey, userId);
